...
"""
    ...
    def roll_back(self, mathproxyagent, assistant):

        self.update_function_call_forall(self.checkpoint_function_json, mathproxyagent, assistant)
        return self.checkpoint_history_list, self.checkpoint_statistic_list, self.checkpoint_function_json

    def update_function_call_forall(self, function_json, mathproxyagent, assistant):
        ...
        for func_json in function_json:
            ...
            self.update_function_call(action, mathproxyagent, assistant)
        ...
        for action_index in range(self._action_num):
            ...
            for _ in range(self._each_action_max_trials):
                ...
                if self._val_json(actions) and self._val_syntax(actions) and self._val_remove(actions, modified_signiture):
                    break
            if actions is not None and self._val_json(actions) and self._val_syntax(actions) and self._val_remove(actions, modified_signiture):
