from utils_react import retry_with_exponential_backoff, normalize_answer, extract_code, trim_string
...
def extract_and_execute_code(code, lang, use_docker, work_dir):
    ...
    return execute_code(code, lang=lang, use_docker=use_docker, work_dir=work_dir, timeout = 10)


class ReActAgent:
    TASK_PROMPT = """Soliving the following mathematics problems using your coding skills. Below is a list of the tools you can use and their detailed descriptions:
{tool_desc}
...
"""
    ...
    def generate_tool_response(self, tool_name, args):
        ...
        if tool_name not in self._function_map.keys():
            return "The action is not valid. It should be excatly one of the following tool names: " + ", ".join(self.get_tool_names())
    ...
    def query(self, task, to_print=False):
        ...
        if answer.endswith('.0'):
            answer = answer[:-2]
        correct = int(final_answer == answer)
